...
Estatero-Classic Core 0.11.0 is forked off the Bitcoin Core 0.9.3. The old darkcoin
...
with the introduced changes to the Estatero-Classic Core 0.11.0.


Estatero-Classic Core:

- Rebrand to `Estatero-Classic Core`
...
- Defined BIP32 (HD) address versions to start with `drkp`/`drkv` (`DRKP`/`DRKV`
  for testnet)
- Adapted BIP44 coin type `5` for Estatero-Classic (0x80000005) as defined in SLIP-0044
...
- Add '-disablewallet' mode to let estaterod run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with estaterod
