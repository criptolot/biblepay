...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in " + CURRENCY_NAME + "/kB\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for transactions in " + CURRENCY_NAME + "/kB\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "dac|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("debug", "dac")
            + HelpExampleRpc("debug", "dac+net")
        );
...
#ifdef ENABLE_WALLET
        // Case 1: Check address and we have full public key:
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* DAC features */
    { CURRENCY_NAME,               "mnsync",                 &mnsync,                 true,  {} },
    { CURRENCY_NAME,               "spork",                  &spork,                  true,  {"value"} },
