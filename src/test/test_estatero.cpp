...
#define BOOST_TEST_MODULE Estatero Test Suite

#include "test_estatero.h"
...
TestingSetup::TestingSetup(const std::string& chainName) : BasicTestingSetup(chainName)
{
    const CChainParams& chainparams = Params();
        ...
        ClearDatadirCache();
        pathTemp = GetTempPath() / strprintf("testestatero_%lu_%i", (unsigned long)GetTime(), (int)(GetRand(100000)));
...
{
    const CChainParams& chainparams = Params();
    std::unique_ptr<CBlockTemplate> pblocktemplate = BlockAssembler(chainparams).CreateNewBlock(scriptPubKey, "", "", 0);
